...
provider, the manifests, both docs -- was written out in full. Here's
exactly how far it was *verified*, so you know what you're picking up:

- **`internal/provider` and `internal/provider/github` are dependency-free
  (standard library only) and their tests actually run and pass** --
  `go test ./internal/provider/...` exercises real RS256 JWT signing
  against a generated test key, a mocked GitHub API (httptest), and
  signature verification against the key's public half.
- **`api/v1alpha1`, `internal/controller`, and `cmd/` need
  `k8s.io/apimachinery`, `k8s.io/client-go`, and `sigs.k8s.io/controller-runtime`**,
  which could not be fetched in the sandbox this project was scaffolded in
  (no network access to the Go module proxy). `zz_generated.deepcopy.go`
  and `config/crd/bases/*.yaml` were hand-written to match what
  `controller-gen` would produce, rather than generated by it.

  To still get real signal without network access, every package in this
  repo -- including these -- was additionally built, vetted, and tested
  against a hand-written, purely local stand-in for those three modules
  (implementing just the API surface this repo actually calls: `ObjectMeta`,
  `client.Client`, the fake client, `controllerutil`, etc.), wired in via
  `go.mod` `replace` directives pointing at local directories instead of
  the network. That stand-in is a throwaway verification harness, not
  something shipped in this repo -- but with it, `go build ./...`,
  `go vet ./...`, and `go test ./...` all genuinely ran and passed: every
  package compiles, and all 17 tests (5 provider + 12 reconciler) pass,
  covering finalizer-before-mint ordering, mint-and-write-secret, rotation
  ...
  None of this replaces the real thing. Run:
  ```
  go mod tidy          # resolves and locks the real dependency graph
  make manifests generate   # regenerates the CRD YAML + deepcopy from controller-gen, once available
  go build ./...
  go test ./...
  ```
  on a machine with normal internet access, against the genuine
  `k8s.io/apimachinery`/`client-go`/`controller-runtime`, before trusting
  this beyond what's described above. `make envtest-setup` gets you a real
  (if lightweight) kube-apiserver + etcd to run integration tests against
  next, with no Docker/kind cluster required.
